...
export class ProductSyncService {
  ...
  async handleSyncProductImagesJob(jobConfiguration: JobConfiguration) {
    const products = await this.productsService.getClientProducts(
      jobConfiguration.tenantId,
    );
    let uploadMediaItems = [];
    for (const product of products.filter(
      (p) => p.main_image_url?.length && p.ean_normalized,
    )) {
  ...
  excludeProductsWithShortExpiryDate(
    ...
    for (const product of products) {
      if (!product.expiration_date) {
        ...
        isDateValidByShelfLife(
          product.expiration_date_normalized,
          minimumShelfLife,
        )
      ) {
        filteredProducts.push(product);
  ...
  async handleSyncProducts(
    ...
    await this.productsService.upserProducts(
      clientId,
      supplierId,
      normalizedProducts,
    );
  ...
  async handleSyncExcelFeedJob(jobConfiguration: JobConfiguration) {
    try {
      ...
      const data = await readAndMapExcel(
        feed_url,
        junkRows,
        productMappingKeys,
      );
  ...
  async handleSyncWebApiJob(jobConfiguration: JobConfiguration) {
    try {
      const { entityReferenceId, tenantId, config } = jobConfiguration;
      const { feed_url, productMappingKeys, authorization, responsePath } =
        config;
      const data = await readWebApiFeed(
        feed_url,
        authorization,
        responsePath,
        productMappingKeys,
