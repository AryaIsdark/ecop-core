...
import {
  EntityType,
  JobConfiguration,
} from 'src/job-configurations/entities/job-configuration.entity';
import { Supplier } from 'src/suppliers/entities/supplier.entity';
import { JobsService } from 'src/jobs/jobs.service';
import { Order } from 'src/orders';
import {
  EcommercePlatform,
  EcommercePlatformsService,
} from 'src/ecommerce-platforms';
import {
  WarehouseManagementSystem,
  WarehouseManagementSystemsService,
} from 'src/warehouse-management-systems';
...
export interface WarehouseManagementSystemJobConfiguration
...
export class ClientsService {
  ...
  async getClientSuppliers(clientId: number): Promise<Supplier[]> {
    const jobConfigurations = await this.jobConfigurationsService.query(
      clientId,
  ...
  async getGeneralJobConfigurations(
    clientId: number,
    ...
    for (const jobConfiguration of jobConfigurations) {
      const jobs = await this.jobsService.search_deprecated({
        entityReferenceId: jobConfiguration.id,
      });
  ...
  async getSupplierJobConfigurations(
    clientId: number,
  ): Promise<SupplierProductSyncJobConfiguration[]> {
    const jobConfigurations = await this.jobConfigurationsService.query(
      clientId,
      { entityType: EntityType.supplier },
    );
  ...
  async getWarehouseManagementSystemJobConfigurations(
    clientId: number,
  ): Promise<WarehouseManagementSystemJobConfiguration[]> {
    const jobConfigurations = await this.jobConfigurationsService.query(
      clientId,
      { entityType: EntityType.warehouseManagemenSystem },
    );
    const mappedJobConfigurations = [];
    for (const jobConfiguration of jobConfigurations) {
      const warehouseManagementSystemId = jobConfiguration.entityReferenceId;
      const warehouseManagementSystem =
        await this.warehouseManagementSystemsService.findOne(
          warehouseManagementSystemId,
        );
      const jobs = await this.jobsService.search_deprecated({
        entityReferenceId: jobConfiguration.id,
      });
      mappedJobConfigurations.push({
        ...jobConfiguration,
        warehouseManagementSystem,
        jobs,
      });
  ...
  async getEcommercePlatofmJobConfigurations(
    clientId: number,
  ): Promise<EcommercePlatformJobConfiguration[]> {
    const jobConfigurations = await this.jobConfigurationsService.query(
      clientId,
      { entityType: EntityType.ecommercePlatform },
    );
    const mappedJobConfigurations = [];
    for (const jobConfiguration of jobConfigurations) {
      const ecommercePlatformId = jobConfiguration.entityReferenceId;
      const ecommercePlatform =
        await this.ecommercePlatformService.findOne(ecommercePlatformId);
      const jobs = await this.jobsService.search_deprecated({
        entityReferenceId: jobConfiguration.id,
      });
      mappedJobConfigurations.push({
  ...
  async getTenantEcommercePlatformOptions(tenantId: number) {
    const ecommercePlatformOptions =
      await this.ecommercePlatformService.findAll();
  ...
  async getTenantReferenceData(tenantId: number) {
    ...
    const tenantEcommercePlatformOptions =
      await this.getTenantEcommercePlatformOptions(tenantId);

    return {
      tenantSupplierOptions,
      tenantEcommercePlatformOptions,
      tenantWmsOptions,
    };
  }
