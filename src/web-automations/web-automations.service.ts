...
import {
  PowerbodyConnectorService,
  PowerbodyWebAutomationConfig,
} from 'src/powerbody-connector/powerbody-connector.service';
...
export class WebAutomationsService {
  ...
  async handleSupplierWebAutomationJob(jobConfiguration: JobConfiguration) {
    ...
    if (supplier.name === 'powerbody') {
      const products =
        await this.powerbodyConnectorService.handleWebAutomationJob(
          jobConfiguration.config as unknown as PowerbodyWebAutomationConfig,
          tenantId,
        );
      if (products?.length) {
        console.log(`${products.length} found, syncing products begins...`);
        await this.productSyncService.handleSyncProducts(
          tenantId,
          entityReferenceId,
          products,
          jobConfiguration,
